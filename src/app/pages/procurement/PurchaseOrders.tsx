  ...
  useEffect(() => {
    (async () => {
      ...
      const { data } = await supabase
        ...
        .select('id, po_number, status, total_amount, created_at, expected_delivery_date, suppliers(name), po_items(quantity)')
              ...
              <tbody>
                {filteredOrders.map((po) => (
                  <StyledTr key={po.id}>
                    ...
                    <StyledTd right mono>{(po.po_items ?? []).reduce((sum, item) => sum + (item.quantity ?? 0), 0)}</StyledTd>
